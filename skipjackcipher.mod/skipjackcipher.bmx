...
ModuleInfo "CC_OPTS: -DLTC_SKIPJACK"

Import "common.bmx"

New TSkipjackCipherFactory(bmx_crypto_skipjack_register())

Type TSkipjackCipher Extends TBlockCipher

	Method KeySize:Int(key:Int) Override
		Return bmx_crypto_skipjack_keysize(key)
	End Method
	...
	Method Setup:Int(key:String, rounds:Int = 0) Override
		...
		Local ret:Int = skipjack_setup(s, Int(strlen_(s)), rounds, keyPtr)
		MemFree(s)
		Return ret
	End Method

	Method Setup:Int(key:Byte[], rounds:Int = 0) Override
		Return skipjack_setup(key, key.length, rounds, keyPtr)
	End Method

	Method Setup:Int(key:Byte Ptr, keylen:Int, rounds:Int = 0) Override
		Return skipjack_setup(key, keylen, rounds, keyPtr)
	End Method

	Method Encrypt:Int(pt:Byte Ptr, ct:Byte Ptr) Override
		Return skipjack_ecb_encrypt(pt, ct, keyPtr)
	End Method

	Method Decrypt:Int(ct:Byte Ptr, pt:Byte Ptr) Override
		Return skipjack_ecb_decrypt(ct, pt, keyPtr)		
	End Method

	Method Done() Override
		skipjack_done(keyPtr)
	End Method

End Type

Type TSkipjackCipherFactory Extends TCipherFactory
	Method Find:TCipher(index:Int) Override
		If index = Self.index Then
			Return New TSkipjackCipher(index)
		End If
	End Method
End Type
