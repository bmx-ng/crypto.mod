...
ModuleInfo "CC_OPTS: -DLTC_NOEKEON"

Import "common.bmx"

New TNoekeonCipherFactory(bmx_crypto_noekeon_register())

Type TNoekeonCipher Extends TBlockCipher

	Method KeySize:Int(key:Int) Override
		Return bmx_crypto_noekeon_keysize(key)
	End Method
	...
	Method Setup:Int(key:String, rounds:Int = 0) Override
		...
		Local ret:Int = noekeon_setup(s, Int(strlen_(s)), rounds, keyPtr)
		MemFree(s)
		Return ret
	End Method

	Method Setup:Int(key:Byte[], rounds:Int = 0) Override
		Return noekeon_setup(key, key.length, rounds, keyPtr)
	End Method

	Method Setup:Int(key:Byte Ptr, keylen:Int, rounds:Int = 0) Override
		Return noekeon_setup(key, keylen, rounds, keyPtr)
	End Method

	Method Encrypt:Int(pt:Byte Ptr, ct:Byte Ptr) Override
		Return noekeon_ecb_encrypt(pt, ct, keyPtr)
	End Method

	Method Decrypt:Int(ct:Byte Ptr, pt:Byte Ptr) Override
		Return noekeon_ecb_decrypt(ct, pt, keyPtr)		
	End Method
...
Type TNoekeonCipherFactory Extends TCipherFactory
	Method Find:TCipher(index:Int) Override
		If index = Self.index Then
			Return New TNoekeonCipher(index)
		End If
	End Method
End Type
